...
class CambiarPasswordControllerTest {
    ...
    void setUp() throws Exception {
        ...
        // lenient: algunos tests no llegan a bundle.getString(...) y Mockito estricto
        // lanzaría UnnecessaryStubbingException si usamos when() normal en setUp()
        ...
        setField("mensajeLabel", mock(Label.class));
    ...
    void initialize_conSesion_noRedirige() {
        try (MockedStatic<Session> sessionMock = mockStatic(Session.class);
            ...
            sessionMock.when(Session::getCurrentUser).thenReturn(usuarioConPassword("hash"));
    ...
    void cambiarPassword_contrasenaActualIncorrecta_noActualiza() {
        try (MockedStatic<Session> sessionMock = mockStatic(Session.class);
            ...
            when(oldPasswordField.getText()).thenReturn("incorrecta");
 
            pwMock.when(() -> PasswordUtils.checkPassword("incorrecta", "hash"))
                    .thenReturn(false);
    ...
    void cambiarPassword_contrasenasNuevasNoCoinciden_noActualiza() {
        try (MockedStatic<Session> sessionMock = mockStatic(Session.class);
            ...
            when(oldPasswordField.getText()).thenReturn("correcta");
            when(newPasswordField.getText()).thenReturn("nueva123");
            when(repeatPasswordField.getText()).thenReturn("diferente");
 
            pwMock.when(() -> PasswordUtils.checkPassword("correcta", "hash"))
                    .thenReturn(true);
    ...
    void cambiarPassword_datosValidos_actualizaYRedirige() {
        try (MockedStatic<Session> sessionMock = mockStatic(Session.class);
            ...
            when(oldPasswordField.getText()).thenReturn("correcta");
            when(newPasswordField.getText()).thenReturn("nuevaPass123");
            when(repeatPasswordField.getText()).thenReturn("nuevaPass123");
 
            pwMock.when(() -> PasswordUtils.checkPassword("correcta", "hash"))
                    .thenReturn(true);
            pwMock.when(() -> PasswordUtils.hashPassword("nuevaPass123"))
